...
export default	{
	...
	methods:	{
		...
		async Upload()
		{
			...
			const ret = await fetch(`http://kittypong.fr:3000/api/users/update/${this.userId}`, {
				method: 'post',
					headers: { 'content-type': 'application/json' },
		...
		async handleTwoFA()
		{
			if (this.twofa == false)
			{
				const res = await fetch('http://kittypong.fr:3000/api/2fa/generate', {
				...
				this.secret = rep;
				const ret = await fetch(`http://kittypong.fr:3000/api/2fa/qrcode`, {
				method: 'get',
					headers: { 'responseType': 'blob' },
				})
				const blob = await ret.blob();
		...
		async turnOn2FA()
		{
			...
			const res = await fetch('http://kittypong.fr:3000/api/2fa/turn-on', {
				method: 'post',
				headers: { 'content-type': 'application/json' },
		...
		async turnOff2FA()
		{
			...
			const res = await fetch('http://kittypong.fr:3000/api/2fa/turn-off', {
				method: 'post',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({code: this.googlecode, user: this.user}),
				})
