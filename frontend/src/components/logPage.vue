<template>
	<div>
		...
		<a class="submit42Button" href="https://localhost:3000/api/oauth2/school42">
...
checkForm() {
		...
		async 	twoFACheck()
		{
			...
			const res = await fetch('https://localhost:3000/api/2fa/authenticate', {
				method: 'post',
				headers: { 'content-type': 'application/json' },
		...
		async	loginWith42(){
			return await fetch(`https://localhost:3000/api/oauth2/school42`, {
						method: 'get',
						mode: 'no-cors',
						headers: { 'content-type': 'application/json' },
			})
