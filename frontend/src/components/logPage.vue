...
checkForm() {
		...
		async login()	{
				...
				if (this.error)
					return ;
				const res = await fetch(`http://kittypong.fr:3000/api/users/login`, {
				method: 'post',
		...
		async 	twoFACheck()
		{
			...
			const res = await fetch('http://kittypong.fr:3000/api/2fa/authenticate', {
				method: 'post',
				headers: { 'content-type': 'application/json' },
		...
		async	loginWith42(){
			return await fetch(`http://kittypong.fr:3000/api/oauth2/school42`, {
						method: 'get',
						mode: 'no-cors',
						headers: { 'content-type': 'application/json' },
			})
